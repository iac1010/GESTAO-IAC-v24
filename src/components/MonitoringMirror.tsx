...
export function MonitoringMirror({ className = '', showLabel = true }: MonitoringMirrorProps) {
  const [isPumpOn, setIsPumpOn] = useState(false);
  const [isGateOpen, setIsGateOpen] = useState(false);

  const handlePumpToggle = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleGateToggle = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    <div className={`relative bg-slate-900/40 backdrop-blur-2xl border border-white/20 p-4 rounded-[2.2rem] shadow-2xl overflow-hidden ${className}`}>
      ...
      <div className="flex gap-3 h-24 relative z-10">
        ...
        <motion.button
          ...
          className={`flex-1 flex flex-col items-center justify-center gap-2 rounded-2xl border transition-all duration-300 ${
            isPumpOn 
