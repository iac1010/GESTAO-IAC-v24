...
export function KanbanMirror({ tickets, className = '', onColumnClick, showLabel = true }: KanbanMirrorProps) {
  return (
    <div className={`relative bg-slate-900/40 backdrop-blur-2xl border border-white/20 p-5 rounded-[2.2rem] shadow-2xl overflow-hidden ${className}`}>
      ...
      <div className="flex gap-4 h-28 relative z-10">
        {COLUMNS.map((col) => {
          const colTickets = tickets.filter(t => (t.status || 'APROVADO') === col.id);
          return (
            <div 
              key={col.id} 
              className="flex-1 flex flex-col gap-2 cursor-pointer group/col"
              onClick={() => onColumnClick?.(col.id)}
            >
              <div className={`h-1.5 rounded-full ${col.color.replace('text-', 'bg-')} shadow-[0_0_10px_rgba(255,255,255,0.1)] opacity-60 group-hover/col:opacity-100 transition-all`} />
              <div className="flex-1 bg-white/5 rounded-2xl p-2 flex flex-col gap-1.5 border border-white/5 group-hover/col:bg-white/10 transition-colors overflow-hidden">
                {colTickets.slice(0, 8).map(t => (
                  <motion.div 
                    key={t.id}
                    layoutId={`mini-${t.id}`}
                    className={`h-1.5 w-full rounded-full shrink-0 shadow-[0_0_8px_rgba(255,255,255,0.2)] transition-all ${
                      t.type === 'TAREFA' 
      ...
      {showLabel && (
        <div className="mt-5 flex justify-between items-center px-1 relative z-10">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/50">Board Mirror</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black text-white/80 bg-white/10 px-2.5 py-1 rounded-lg border border-white/10 backdrop-blur-md">
              {tickets.length}
            </span>
          </div>
        </div>
